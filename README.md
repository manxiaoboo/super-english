...
- `ai-active-chunk-practice`：把生硬表达和自然改写沉淀为“主动表达词块”，并通过词块 + 句型骨架 + 现实场景进行输出训练。你输入完整英文句子后，系统会在诊断后自动提炼并保存高价值主动词块。
- `ai-vocabulary-review`：读取已保存的单词、短语、语法和句子学习记录，按复习日期生成主动回忆测试，并更新复习记录。
- `ai-grammar-course`：以张满胜《英语语法新思维》为教材，从零系统学习英语语法。每节课聚焦一个"语法思维"而非死记规则，配合结构图解、最小对比和主动造句练习。支持学习进度追踪和间隔复习。
- `ai-production-diagnostic-report`：基于累计的造句诊断记录，生成阶段性能力缺口分析和训练建议。
- `ai-grammar-translation-drill`：基于张满胜语法思维的疯狂造句模式。AI 出中文句子，用户翻译成英文，诊断纠错并自动记录正确版本供朗读训练使用。场景固定在外企程序员日常工作（代码审查、站会、部署、1on1 等），覆盖已学30章全部语法思维点。
...
5. 主动词块：调用 `ai-active-chunk-practice`，把自然改写中的高价值表达保存为可复用词块并训练输出；当你主动写完整英文句子时，AI 会在纠错后自动保存可复用词块。
6. 每日复习：调用 `ai-vocabulary-review`，让 AI 复习今天到期的所有学习项。
7. 造句诊断：当你回答问题、自己造句或尝试翻译时，AI 会隐式记录高频表达问题。
8. 更新进度：根据回答结果标记 `forgot`、`hard`、`good` 或 `easy`，并更新下次复习日期。
...
```text
/ai-active-chunk-practice 主动词块训练 5 个
```
